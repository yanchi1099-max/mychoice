...
import { Macros, RecommendationRequest, RecommendationType, AiResponse, FoodItem, DailyRecord } from "../types";
...
export const getMealRecommendation = async (
  ...
  const remainingStr = `
    ...
    剩余配额: 热量 ${request.remainingMacros.calories}, 蛋白质 ${request.remainingMacros.protein}g, 碳水 ${request.remainingMacros.carbs}g
  `;

  if (request.type === RecommendationType.DECISION && request.userOptions) {
    prompt = `
      用户在纠结：${JSON.stringify(request.userOptions)}。
      ${remainingStr}
      请选择最合适的一项。
    ...
    prompt = `
      用户需要一个${request.targetMealName}的推荐。
      ${remainingStr}
      请推荐一顿简单、健康且美味的餐食。
...
export const analyzeManualEntry = async (text: string, imageBase64?: string): Promise<FoodItem[]> => {
  const model = "gemini-2.5-flash"; // Use generic 2.5 flash which supports images
  ...
  try {
    ...
    if (imageBase64) {
      parts.push({
        inlineData: {
          mimeType: 'image/jpeg', // Assuming JPEG for simplicity, usually safe for generic base64
          data: imageBase64
        }
      });
    }
    ...
    if (result.text) {
      return JSON.parse(result.text) as FoodItem[];
    }
    return [];
  } catch (error) {
